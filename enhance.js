...
function renderItemDetail(itemId) {
  ...
  const titleHtml = `
    <div class="detail-head ${it.bcls}">
      ...
      <div class="muted" style="font-size:11px">${SLOT_INFO[it.slot].label} · [${it.rarityName}]${it.reqLvl?' · Lv.'+it.reqLvl:''}${found.source==='equip'?' · <span style="color:var(--accent)">已装备</span>':''}</div>
    </div>`;
  // 基础属性
...
function itemBonusSummary(item) {
  ...
  return parts.length ? ' <span class="muted" style="font-size:10px">'+parts.join(' ')+'</span>' : '';
}
