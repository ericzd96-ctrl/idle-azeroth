...
{key:'tal_b0z1v9', name:'壁垒', desc:'防御 +8%/层 · 生命 +5%/层', max:5,req:38,mod:{defPct:8,hpPct:5}},{key:'tal_tjz4f9', name:'盾牌反震', desc:'反伤 +6%/层 · 耐力 +6%/层', max:5,req:42,mod:{reflectDmg:6,staPct:6}},{key:'tal_05he3a', name:'钢铁守护者', desc:'防御 +12%/层 · 生命 +8%/层 · 精通 +2%/层', max:3,req:45,mod:{defPct:12,hpPct:8,mastery:2}}]},
  ...
  mage: {
    ...
    skills:{
      ...
      polymorph:    {name:'变形术',   icon:'🐑', desc:'3倍攻击,变形击晕并顺带打断读条', mp:30, type:'dmg', mul:3, slow:true, stun:true, stunMs:1500, interruptCast:true, unlockLvl:35,castTime:1.5,cd:18},
...
{key:'tal_4sxjog', name:'冰河时代', desc:'防御 +8%/层 · 全能 +4%/层', max:5,req:38,mod:{defPct:8,vers:4}},{key:'tal_4bba7s', name:'绝对零度', desc:'暴伤 +10%/层 · 额外攻击 +1%/层', max:5,req:42,mod:{critdPct:10,extraAtk:4}},{key:'tal_815u80', name:'大法师', desc:'攻击 +1%/层 · 暴击 +3%/层 · 精通 +2%/层', max:3,req:45,mod:{atkPct:4,crit:3,mastery:2}}]},
  ...
  priest: {
    ...
    skills:{
      ...
      shackleUndead:{name:"束缚亡灵",icon:"⛓️",desc:"4倍伤害,束缚击晕并顺带打断读条",mp:28,type:"dmg",mul:4,slow:true,stun:true,stunMs:1300,interruptCast:true,unlockLvl:44,cd:18},
...
{key:'tal_olt688', name:'暗影幻灵', desc:'持续伤害 +8%/层 · 吸血 +3%/层', max:5,req:38,mod:{dotBonus:8,leech:3}},{key:'tal_wcgwwu', name:'虚空', desc:'攻击 +2%/层 · 暴击 +3%/层', max:5,req:42,mod:{atkPct:2,crit:3}},{key:'tal_bf4hdm', name:'暗影祭司', desc:'攻击 +1%/层 · 斩杀加成 +8%/层 · 精通 +2%/层', max:3,req:45,mod:{atkPct:4,executeBonus:8,mastery:2}}]},
  ...
  rogue: {
    ...
    skills:{
      ...
      kidneyShot:  {name:'肾击',     icon:'👊', desc:'4倍攻击,击晕并顺带打断读条', mp:32, type:'dmg', mul:4, slow:true, stun:true, stunMs:1400, interruptCast:true, unlockLvl:32, cd:18},
...
{key:'tal_5u8399', name:'荒野求生', desc:'生命 +8%/层 · 全能 +4%/层', max:5,req:38,mod:{hpPct:8,vers:4}},{key:'tal_20kiyb', name:'陷阱之王', desc:'攻击 +2%/层 · 反伤 +4%/层', max:5,req:42,mod:{atkPct:2,reflectDmg:4}},{key:'tal_yhyekb', name:'生存至尊', desc:'攻击 +1%/层 · 防御 +8%/层 · 精通 +2%/层', max:3,req:45,mod:{atkPct:4,defPct:8,mastery:2}}]},
  ...
  shaman: {
    ...
    skills:{
      ...
      hex:{name:"妖术",icon:"🐸",desc:"3倍伤害,妖术击晕并顺带打断读条",mp:32,type:"dmg",mul:3,slow:true,stun:true,stunMs:1400,interruptCast:true,unlockLvl:42,cd:18},
...
{key:'tal_qw36s6', name:'生命之泉', desc:'治疗 +8%/层 · 回复 +3%/层', max:5,req:38,mod:{healBonus:8,regFlat:3}},{key:'tal_sz1bbs', name:'先祖智慧', desc:'精神 +8%/层 · 治疗 +6%/层', max:5,req:42,mod:{spiPct:8,healBonus:6}},{key:'tal_sy9xob', name:'灵魂行者', desc:'治疗 +15%/层 · 全能 +5%/层 · 精通 +2%/层', max:3,req:45,mod:{healBonus:15,vers:5,mastery:2}}]},
  ...
  paladin: {
    ...
    skills:{
      ...
      hammerOfJustice:{name:'制裁之锤', icon:'🔨', desc:'3倍神圣伤害,击晕并顺带打断读条', mp:32, type:'dmg', mul:3, stun:true, stunMs:1500, interruptCast:true, unlockLvl:32, castTime:0, cd:18},
...
{key:'tal_v97irx', name:'神圣复仇', desc:'攻击 +2%/层 · 暴伤 +6%/层', max:5,req:38,mod:{atkPct:2,critdPct:6}},{key:'tal_j4xnxx', name:'圣光之怒', desc:'额外攻击 +1%/层 · 斩杀加成 +5%/层', max:5,req:42,mod:{extraAtk:5,executeBonus:5}},{key:'tal_6df7gn', name:'复仇天使', desc:'攻击 +1%/层 · 暴击 +3%/层 · 精通 +2%/层', max:3,req:45,mod:{atkPct:4,crit:3,mastery:2}}]},
  ...
  warlock: {
    ...
    skills:{
      ...
      shadowFury:{name:"暗影之怒",icon:"💢",desc:"5倍范围伤害,震慑并顺带打断读条",mp:42,type:"dmg",mul:5,slow:true,stun:true,stunMs:1200,interruptCast:true,unlockLvl:44,weaken:true,cd:18},
...
{key:'tal_b7fe6p', name:'混沌之雨', desc:'攻击 +2%/层 · 暴伤 +6%/层', max:5,req:38,mod:{atkPct:2,critdPct:6}},{key:'tal_4qj3im', name:'末日降临', desc:'斩杀加成 +8%/层 · 技能冷却 +5%/层', max:5,req:42,mod:{executeBonus:8,cdReduction:5}},{key:'tal_hw275i', name:'毁灭之王', desc:'攻击 +1%/层 · 暴击 +4%/层 · 精通 +2%/层', max:3,req:45,mod:{atkPct:4,crit:4,mastery:2}}]},
  ...
  druid: {
    ...
    skills:{
      ...
      entanglingRoots:{name:'纠缠根须',icon:'🌿', desc:'3倍自然伤害,缠绕击晕并顺带打断读条', mp:28, type:'dmg', mul:3, slow:true, stun:true, stunMs:1200, interruptCast:true, unlockLvl:32,castTime:1.5,cd:18},
...
const MAPS = [
  ...
  { key:'tirisfal', name:'提瑞斯法林地', icon:'⚰️', faction:'部落', lvlRange:[1,10],
    ...
    sub:[
      { name:'丧钟镇外',     lvl:[1,4],  mobs:'🐺枯草狼|🕷️瘟疫蜘蛛|🦅腐肉啄食者|🐀瘟疫鼠' },
      { name:'布瑞尔墓园',   lvl:[5,7],  mobs:'💀叛逃卫兵|👻闪光怨灵|🧟黏滑污泥|🦴白骨爬虫' },
      { name:'壁垒哨线',     lvl:[8,10], mobs:'👤叛逃斥候|🧙堕落法师|🐺月夜狼|💀亡灵步兵' },
    ],
  ...
  { key:'durotar', name:'杜隆塔尔', icon:'🌵', faction:'部落', lvlRange:[1,10],
    desc:'兽人在卡利姆多的红色家园',
    sub:[
      { name:'试炼谷',     lvl:[1,4],  mobs:'🐗峡谷野猪|🦂沙漠蝎子|👹火刃斥候|🐺旱地野狼' },
      { name:'森金村外',   lvl:[5,7],  mobs:'🐊海岸鳄鱼|🦀回音海蟹|👹火刃信徒|🦂沙漠蝎子' },
      { name:'剃刀岭',     lvl:[8,10], mobs:'👹火刃狂徒|🐗剃刀岭野猪|🦂沙漠蝎子|🌪️风暴元素' },
    ],
  ...
  { key:'shadowmoon', name:'影月谷', icon:'🌑', faction:'外域', lvlRange:[67,70],
    ...
    boss:{ name:'伊利达雷守门官', emoji:'😈', lvl:72, desc:'把守黑暗神殿外墙的伊利达雷军官',
      ...
      skills:[{name:"魔刃突袭",icon:"🗡️",desc:"7倍伤害",type:"dmg",mul:7,castTime:2},{name:"恶魔变形",icon:"😈",desc:"8倍范围暗影伤害",type:"dmg",mul:8,castTime:4},{name:'眼棱',icon:'👁️',desc:'7倍火焰伤害',type:'dmg',mul:7,castTime:2.5},{name:'法力燃烧',icon:'🔥',desc:'6倍伤害+灼烧',type:'dmg',mul:6,dot:true,castTime:3}] } },
  ...
  { key:'icecrown', name:'冰冠冰川', icon:'🏰', faction:'诺森德', lvlRange:[78,80],
    ...
    sub:[
      ...
      { name:'堡垒外墙', lvl:[79,80], mobs:'🧟瘟疫食尸鬼|🦴骨爬行者|💀亡灵法师|🐲霜龙' },
    ],
    boss:{ name:'天灾攻城统领', emoji:'☠️', lvl:83, desc:'把守冰冠堡垒外侧封锁线的天灾军官' ,
      ...
      skills:[{name:"符文战刃",icon:"🗡️",desc:"7倍伤害+吸血30%",type:"dmg",mul:7,lifeSteal:0.3,castTime:2},{name:"死亡缠绕",icon:"💀",desc:"8倍暗影伤害",type:"dmg",mul:8,castTime:3},{name:"寒冰风暴",icon:"❄️",desc:"8倍范围+减速",type:"dmg",mul:8,slow:true,castTime:3.5},{name:"亡者大军",icon:"🧟",desc:"9倍伤害",type:"dmg",mul:9,castTime:4,stun:true},{name:"天灾军令",icon:"👑",desc:"10倍范围暗影伤害",type:"dmg",mul:10,castTime:5}] } },
  ...
  { key:'dragonblight', name:'龙骨荒野', icon:'🐉', faction:'诺森德', lvlRange:[72,76],
    ...
    boss:{ name:'霜骨龙将', emoji:'🐉', lvl:78, desc:'盘踞龙骨荒野的天灾霜龙' ,
...
/* 英雄级 5 人副本:仿史诗团本,给较高级 5 人本(reqLvl>=HEROIC_MIN_REQ)自动生成高难度版
   (<key>_heroic)。掉落复用基础本(baseDungeonKey 已剥离 _heroic),但按基础副本进度继续线性缩放。 */
const HEROIC_MIN_REQ = 60;
/* 英雄本等级按基础本梯队:基础 60~70 → 70 级,71~80 → 80 级,80 后沿用终局进度,不再降回 80。 */
function heroicTierLevel(baseReqLvl) {
  const req = Math.max(HEROIC_MIN_REQ, Math.floor(baseReqLvl || HEROIC_MIN_REQ));
  if (req > 80) return req;
  return req >= 71 ? 80 : 70;
}
function highDungeonVariantPower(baseReqLvl, mode) {
  const req = Math.max(HEROIC_MIN_REQ, Math.floor(baseReqLvl || HEROIC_MIN_REQ));
  if (req > 80) return Math.round(req + (mode === 'epic5' ? 5 : 2));
  const highTier = req >= 71;
  const tierBase = highTier ? 80 : 70;
  const span = highTier ? Math.max(0, req - 71) : Math.max(0, req - HEROIC_MIN_REQ);
  const drift = Math.min(highTier ? 9 : 10, span);
  const modeBonus = mode === 'epic5' ? 4 : 0;
  const step = mode === 'epic5' ? 0.55 : 0.40;
  return Math.round(tierBase + modeBonus + drift * step);
}
function createHeroicDungeonCatalog() {
  ...
  for (const base of bases) {
    ...
    const powerLvl = highDungeonVariantPower(base.reqLvl, 'heroic');
    const clone = JSON.parse(JSON.stringify(base));
    clone.key = key;
    clone.baseKey = base.key;
    clone.name = `${base.name}·英雄`;
    clone.desc = `${tierLvl}级英雄5人本 · 战斗强度 ${powerLvl} · ${base.name} 的高难度版本`;
    clone.reqLvl = tierLvl;
    clone.powerLvl = powerLvl;
    clone.sortPower = powerLvl + 0.1;
...
/* 史诗5人本(Mythic 难度):给 reqLvl>=EPIC5_MIN_REQ 的基础 5 人本自动生成史诗版
   (<key>_epic5)。难度/掉落梯队介于英雄(tier1)与团本(tier2)之间(gearTier=4,×1.28)。
...
function createEpic5DungeonCatalog() {
  ...
  for (const base of bases) {
    ...
    const powerLvl = (typeof highDungeonVariantPower === 'function') ? highDungeonVariantPower(base.reqLvl, 'epic5') : (tierLvl + 4);
    const clone = JSON.parse(JSON.stringify(base));
    clone.key = key;
    clone.baseKey = base.key;
    clone.name = `${base.name}·史诗`;
    clone.desc = `${tierLvl}级史诗5人本 · 战斗强度 ${powerLvl} · ${base.name} 的极限难度版本(最强5人内容)`;
    clone.reqLvl = tierLvl;
    clone.powerLvl = powerLvl;
    clone.sortPower = powerLvl + 0.4;
    clone.cd = Math.max(base.cd || 0, 1500);
    clone.epic5 = true;
    clone.bosses = (clone.bosses || []).map((boss, idx, arr) => {
      const isFinal = idx === arr.length - 1;
