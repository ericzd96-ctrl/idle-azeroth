...
function combatRecentSkillAmountMeta(item) {
  const damage = Math.floor(item?.damage || 0);
  const heal = Math.floor(item?.heal || 0);
  const shield = Math.floor(item?.shield || 0);
  const taken = Math.floor(item?.taken || 0);
  if (heal > 0) return { kind:'heal', short:'+' + fmt(heal), label:'治疗 ' + fmt(heal), title:'累计治疗 ' + fmt(heal) };
...
function combatRecentSkillDetailText(item) {
  const amount = combatRecentSkillAmountMeta(item);
  const max = Math.floor(item?.maxAmount || 0);
  const crits = item?.crits || 0;
  const parts = [];
  if (amount) parts.push(amount.title);
  if (max > 0) parts.push('最高一跳 ' + fmt(max));
...
function updateStageSkillChain(list, now) {
  ...
  fresh.forEach((item, idx) => {
    ...
    result.textContent = amount ? amount.short : (danger ? '高危' : (combatRecentSkillHitText(item) || combatRecentSkillEffectText(item)));
...
function updateDmgRecentSkills() {
  ...
  list.forEach((item, idx) => {
    ...
    chip.className = `dm-recent-chip actor-${actor} school-${school} type-${type}${amount ? ' has-amount amount-' + amount.kind : ''}${idx === 0 ? ' is-latest' : ''}${danger ? ' is-danger' : ''}${ageOld ? ' is-old' : ''}`;
    ...
    if (amount) {
      ...
      amountEl.className = `dm-recent-amount ${amount.kind}`;
      amountEl.textContent = amount.short;
...
function executeWindowState() {
  const mon = state?.currentMonsters?.find(m => m && m.hp > 0);
  if (!mon || !(mon.hpMax > 0)) return null;
  const pct = Math.max(0, mon.hp / mon.hpMax);
  const threshold = mon.isBoss || mon.isWorldBoss ? 0.4 : 0.35;
  if (pct > threshold) return null;
  return {
    mon,
    pct,
    threshold,
    urgency:Math.max(0, Math.min(100, (threshold - pct) / Math.max(0.01, threshold) * 100))
  };
}
function executeSkillIsFinisher(skillKey, sk) {
  if (!sk || sk.type !== 'dmg') return false;
  const text = `${skillKey || ''} ${sk.name || ''} ${sk.desc || ''}`;
  return sk.consumeRage || sk.executeBonus || sk.ai?.priorityTag === 'execute' || sk.ai?.useIfTargetHpBelow || /斩杀|处决|终结|收割|灭杀|杀戮射击|暗言术·灭|死亡标记|终极审判|湮灭|喉切/.test(text);
}
function executeSkillScore(skillKey, sk) {
  if (!sk) return 0;
  const text = `${skillKey || ''} ${sk.name || ''} ${sk.desc || ''}`;
  return (sk.mul || 1) * 100
    + (sk.executeBonus || 0) * 3
    + (sk.consumeRage ? 110 : 0)
    + (sk.alwaysCrit ? 42 : 0)
    + ((sk.ai?.priorityTag === 'execute') ? 95 : 0)
    + ((sk.ai?.useIfTargetHpBelow || 0) > 0 ? 55 : 0)
    + (/斩杀|处决|灭杀|湮灭|喉切/.test(text) ? 70 : 0)
    + (/终结|收割|杀戮射击|暗言术·灭|死亡标记|终极审判/.test(text) ? 50 : 0);
}
function executeSkillThreshold(sk, win) {
  if (sk?.executeThreshold) return sk.executeThreshold;
  if (sk?.ai?.useIfTargetHpBelow) return sk.ai.useIfTargetHpBelow;
  if (sk?.executeBonus) return 0.35;
  return win?.threshold || 0.35;
}
function executeRecommendedSkillKey(now) {
  const win = executeWindowState();
  if (!win) return '';
  const c = getCls();
  let bestKey = '', bestScore = -Infinity;
  for (const key of (state.selectedSkills || [])) {
    const sk = c?.skills?.[key];
    if (!executeSkillIsFinisher(key, sk)) continue;
    if (win.pct > executeSkillThreshold(sk, win)) continue;
    const cdMs = Math.max(0, (state.skillCooldowns?.[key] || 0) - now);
    const cost = skillUiCost(key, sk, c);
    if (cdMs > 0 || (state.resource || 0) < cost) continue;
    const score = executeSkillScore(key, sk);
    if (score > bestScore) {
      bestScore = score;
      bestKey = key;
    }
...
function executeSkillPrompt(skillKey, sk, now, cdMs) {
  if (!executeSkillIsFinisher(skillKey, sk)) return null;
  const win = executeWindowState();
  if (!win) return null;
  const threshold = executeSkillThreshold(sk, win);
  if (win.pct > threshold) return null;
  const cost = skillUiCost(skillKey, sk);
  const ready = (cdMs || 0) <= 0 && (state.resource || 0) >= cost;
  const recommended = ready && executeRecommendedSkillKey(now) === skillKey;
  const target = win.mon.bossName || win.mon.name || '目标';
  const hpText = Math.max(1, Math.round(win.pct * 100));
  const label = ready ? '斩杀' : (cdMs > 0 ? `${Math.ceil(cdMs / 1000)}秒` : '缺资源');
  const cls = ready ? 'execute-hot' : 'execute-wait';
  const tip = ready
    ? `${target} 生命约 ${hpText}%,${recommended ? '推荐用这个收尾技能。' : '可以交斩杀/终结技能。'}`
    : `${target} 已进入斩杀窗口,但该技能${cdMs > 0 ? '还在冷却' : '资源不足'}。`;
  return { label, cls, tip, final:win.pct <= 0.18, recommended, timerPct:win.urgency };
}

function survivalWindowState() {
  const h = state?.hero;
  if (!h || !(h.hpMax > 0)) return null;
  const pct = Math.max(0, Math.min(1, (state.hp || 0) / h.hpMax));
  ...
  return {
    ...
    critical:pct <= 0.12,
    final:pct <= 0.20,
    severity:Math.max(0, Math.min(100, (0.35 - pct) / 0.35 * 100))
  };
}
function survivalSkillScore(skillKey, sk, win) {
  const kind = bossCastResponseKind(skillKey, sk);
  if (!kind) return -Infinity;
  const text = `${skillKey || ''} ${sk?.name || ''} ${sk?.desc || ''} ${sk?.buff || ''}`;
  const healPower = (sk?.healPct || 0) * 130 + (sk?.heal || 0) * 0.03;
  const shieldPower = (sk?.shieldPct || 0) * 115 + (/护盾|屏障|壁垒|守护|圣盾/.test(text) ? 32 : 0);
  const defensivePower = (/减伤|防御|格挡|闪避|盾墙|树皮|冰箱|庇护/.test(text) ? 44 : 0) + Math.max(0, 24 - (sk?.cd || 0)) * 1.5;
  const lowHpBias = win?.critical ? 95 : (win?.final ? 55 : 20);
  return (kind === 'heal' ? 160 : 120)
    + (kind === 'heal' ? healPower + lowHpBias : shieldPower + defensivePower + lowHpBias * 0.72)
    + (/圣疗|宁静|生命|愈合|治疗/.test(text) ? 30 : 0)
    + (/盾|壁垒|守护|圣盾/.test(text) ? 24 : 0);
}
function survivalRecommendedSkillKey(now) {
  const win = survivalWindowState();
  if (!win) return '';
  const c = getCls();
  let bestKey = '', bestScore = -Infinity;
  for (const key of (state.selectedSkills || [])) {
    const sk = c?.skills?.[key];
    if (!sk || !bossCastResponseKind(key, sk)) continue;
    const cdMs = Math.max(0, (state.skillCooldowns?.[key] || 0) - now);
    const cost = skillUiCost(key, sk, c);
    if (cdMs > 0 || (state.resource || 0) < cost) continue;
    const score = survivalSkillScore(key, sk, win);
    if (score > bestScore) {
      bestScore = score;
      bestKey = key;
    }
...
function survivalSkillPrompt(skillKey, sk, now, cdMs) {
  const win = survivalWindowState();
  if (!win || !sk) return null;
  const kind = bossCastResponseKind(skillKey, sk);
  if (!kind) return null;
  const cost = skillUiCost(skillKey, sk);
  const ready = (cdMs || 0) <= 0 && (state.resource || 0) >= cost;
  const recommended = ready && survivalRecommendedSkillKey(now) === skillKey;
  const hpText = Math.max(1, Math.round(win.pct * 100));
  const label = ready
    ? (win.critical ? '急救' : (kind === 'heal' ? '治疗' : '保命'))
    : (cdMs > 0 ? `${Math.ceil(cdMs / 1000)}秒` : '缺资源');
  const cls = ready ? (kind === 'heal' ? 'heal-hot' : 'defensive-hot') : (kind === 'heal' ? 'heal-wait' : 'defensive-wait');
  const action = ready
    ? (kind === 'heal'
      ? `${recommended ? '推荐现在点击。' : '现在可以点击。'}恢复生命,避免被下一次伤害带走。`
      : `${recommended ? '推荐现在点击。' : '现在可以点击。'}开启减伤或护盾,稳住血线。`)
    : (cdMs > 0 ? `暂时不能用:还差 ${Math.ceil(cdMs / 1000)}秒。` : '暂时不能用:资源不足。');
  return {
    label,
    cls,
    tip:`生命约 ${hpText}%。${action}`,
    final:win.final,
    urgent:win.critical,
    recommended,
...
function renderSkillBar() {
  ...
  bar.innerHTML = state.selectedSkills.map(key => {
    ...
    const executePromptClass = executePrompt ? `execute-window-prompt ${executePrompt.cls || ''} ${executePrompt.final ? 'execute-window-final' : ''}` : '';
...
function updateSkillBarCd() {
  ...
  bar.querySelectorAll('.skill-btn').forEach(btn => {
    ...
    btn.classList.toggle('execute-window-prompt', !!executePrompt);
    btn.classList.toggle('execute-window-final', !!executePrompt?.final);
    btn.classList.toggle('execute-window-recommended', !!executePrompt?.recommended);
    btn.classList.toggle('execute-hot', executePrompt?.cls === 'execute-hot');
    btn.classList.toggle('execute-wait', executePrompt?.cls === 'execute-wait');
