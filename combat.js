...
function skillImpactMeta(opts){
  const forcedTag = String(opts?.impactTag || '').trim();
  const forcedGrade = String(opts?.impactGrade || '').replace(/[^a-z0-9_-]/gi, '').toLowerCase();
  ...
  let grade = forcedGrade || 'normal';
  let tag = forcedTag;
  if(!forcedGrade){
    if(opts?.actor === 'boss' && (opts?.empowered || opts?.threat === 'extreme' || ratio >= 0.20)) grade = 'danger';
    else if(opts?.crit) grade = ratio >= 0.16 ? 'huge' : 'crit';
    else if(ratio >= 0.22) grade = 'huge';
    else if(ratio >= 0.12) grade = 'heavy';
  }
  if(!tag){
    if(opts?.empowered) tag = '灭团技';
    else if(opts?.actor === 'boss' && (opts?.threat === 'high' || opts?.threat === 'extreme' || grade === 'danger')) tag = '高危';
    else if(grade === 'huge') tag = '重击';
    else if(grade === 'heavy') tag = '强击';
    else if(grade === 'crit') tag = '暴击';
  }
  return { grade, tag };
}
...
function recordCombatSkillCast(sk, opts){
  ...
  _combatRecentSkillCasts.unshift({
    ...
    interruptPolicy:String(sk.interruptPolicy || ''),
...
function showSkillImpactFx(sourceEl, targetEl, sk, opts){
  ...
  if(impact.tag && opts?.tag !== false){
    ...
    tag.style.top = (b.y - size * 0.72 - 12) + 'px';
    tag.style.setProperty('--skill-fx-duration', (opts?.tagDuration || opts?.duration || 560) + 'ms');
...
function bossTelegraphTargets(cast, mon, now){
  if(!cast) return [];
  if(!bossCastIsDamage(cast)) return mon ? [{ el:monsterFloatAnchor(mon), label:'自施', kind:'self' }] : [];
  if(cast.aoe){
    const out = [{ el:$('hero-emoji'), label:'全体', kind:'aoe' }];
    if(typeof companionTargetable === 'function' && companionTargetable()) out.push({ el:$('comp-mini'), label:'全体', kind:'aoe' });
    if(typeof livingAllySummons === 'function' && typeof allySummonAnchor === 'function'){
      for(const unit of livingAllySummons(now || Date.now()).slice(0, 2)) out.push({ el:allySummonAnchor(unit), label:'全体', kind:'aoe' });
    }
    return out;
  }
  const target = cast._target;
  if(target?.kind === 'companion') return [{ el:$('comp-mini'), label:'点名', kind:'target' }];
...
function showBossTargetTelegraph(cast, mon, opts){
  ...
  if(!targets.length) return;
  const dangerous = cast._empowered || cast.threat === 'high' || cast.threat === 'extreme';
  const duration = opts?.duration || Math.max(700, Math.min(2600, cast.duration || 900));
  const school = skillVisualSchool(null, cast, 'boss');
  for(const t of targets.slice(0, 4)){
    const p = skillFxPoint(t.el);
    if(!p) continue;
    const size = Math.round(Math.max(38, Math.min(92, Math.max(p.w, p.h) + (dangerous ? 28 : 18))));
    const el = document.createElement('div');
    el.className = `boss-target-telegraph ${dangerous ? 'danger' : ''} ${t.kind || 'target'} school-${school}`;
    el.style.left = (p.x - size / 2) + 'px';
    el.style.top = (p.y - size / 2) + 'px';
    el.style.width = size + 'px';
    el.style.height = size + 'px';
    el.style.setProperty('--boss-telegraph-duration', duration + 'ms');
    const tag = document.createElement('span');
    tag.textContent = t.label || '点名';
    el.appendChild(tag);
    layer.appendChild(el);
...
function showBossCastRhythmFx(cast, mon, opts){
  if(!cast || !mon || !bossCastIsDamage(cast) || typeof document === 'undefined' || document.hidden) return;
  const layer = skillFxLayer();
  if(!layer) return;
  const now = opts?.now || Date.now();
  const duration = Math.max(1, cast.duration || 0);
  const pct = typeof opts?.pct === 'number' ? opts.pct : Math.min(100, Math.max(0, (now - (cast.startTime || now)) / duration * 100));
  const remainMs = typeof opts?.remainMs === 'number' ? opts.remainMs : Math.max(0, duration - (now - (cast.startTime || now)));
  const dangerous = cast._empowered || cast.threat === 'high' || cast.threat === 'extreme';
  const finalBeat = remainMs <= (dangerous ? 950 : 700) || pct >= (dangerous ? 68 : 82);
  if(pct < (dangerous ? 18 : 34) && !finalBeat) return;
  const maxBeats = dangerous ? 5 : 3;
  if((cast._rhythmFxCount || 0) >= maxBeats) return;
  const interval = finalBeat ? (dangerous ? 260 : 420) : (dangerous ? 460 : 680);
  if((cast._nextRhythmFxAt || 0) > now) return;
  cast._nextRhythmFxAt = now + interval;
  cast._rhythmFxCount = (cast._rhythmFxCount || 0) + 1;
  const targets = bossTelegraphTargets(cast, mon, now).filter(x => x && x.el).slice(0, 4);
  if(!targets.length) return;
  const school = skillVisualSchool(null, cast, 'boss');
  const label = finalBeat
    ? (cast.interruptPolicy === 'none' ? '硬吃' : '快断')
    : (cast.aoe ? '范围' : '点名');
  const fxDuration = finalBeat ? (dangerous ? 520 : 460) : 420;
  for(const t of targets){
    const p = skillFxPoint(t.el);
    if(!p) continue;
    const size = Math.round(Math.max(34, Math.min(86, Math.max(p.w, p.h) + (finalBeat ? 26 : 16))));
    const el = document.createElement('div');
    el.className = `boss-cast-rhythm ${dangerous ? 'danger' : ''} ${finalBeat ? 'final' : ''} ${t.kind || 'target'} school-${school}`;
    el.style.left = (p.x - size / 2) + 'px';
    el.style.top = (p.y - size / 2) + 'px';
    el.style.width = size + 'px';
    el.style.height = size + 'px';
    el.style.setProperty('--boss-rhythm-duration', fxDuration + 'ms');
    const tag = document.createElement('span');
    tag.textContent = label;
    el.appendChild(tag);
    layer.appendChild(el);
    if(typeof pulseCombatEl === 'function') pulseCombatEl(t.el, finalBeat || dangerous ? 'danger' : 'bosscast', finalBeat ? 300 : 220);
...
function healCompanionAmount(amount, icon, color, source, skillLabel){
  ...
  if(applied > 0){
    ...
    if(before / Math.max(1, st.hpMax || 1) <= 0.32 && state._compHp / Math.max(1, st.hpMax || 1) >= 0.42) combatCueToast('随从稳住', '+' + fmt(applied), 'heal');
  }
  markDirty('companion');
  return { applied, overheal };
}
    ...
    if(elapsed>=bossCasting.duration){
      ...
      if(bc.type==='heal'){const h=bossSkillHealAmount(mon, bc.heal||0.2);mon.hp=Math.min(mon.hpMax,mon.hp+h);showMonsterFloat(mon,'💚+'+h,'#6ee7b7');if(bossCastEl)showSkillSelfFx(bossCastEl,bc,{actor:'boss',school:'heal',pulse:'heal',duration:520});bossCastResultCue(bc,'support',(bc.icon||'')+(bc.name||'治疗')+' · 自身恢复 '+fmt(h),{intensity:.78});}
        ...
        if(bc.aoe){
          ...
          if(bossCastEl) showSkillImpactFx(bossCastEl, $('hero-emoji'), bc, { actor:'boss', amount:taken, targetMax:state.hero.hpMax, threat:bc.threat, empowered:bc._empowered, scale:1.08, pulse:'danger' });
          ...
          if(companionTargetable()){
            ...
            if(bossCastEl) showSkillImpactFx(bossCastEl, $('comp-mini'), bc, { actor:'boss', amount:ct, targetMax:cst?.hpMax || state.hero.hpMax, threat:bc.threat, empowered:bc._empowered, scale:0.9, pulse:'danger' });
          ...
          if(target.kind==='companion'){
            ...
            if(bossCastEl) showSkillImpactFx(bossCastEl, $('comp-mini'), bc, { actor:'boss', amount:ct, targetMax:cst?.hpMax || state.hero.hpMax, threat:bc.threat, empowered:bc._empowered, scale:0.96, pulse:'danger' });
    ...
    if(isAOE){
      ...
      for(const target of state.currentMonsters){
        ...
        if(heroCastEl){
          ...
          showSkillImpactFx(heroCastEl, targetEl, sk, { skillKey, actor:'hero', amount:dd, targetMax:target.hpMax, crit:d.crit || forceCrit, scale:strong ? (d.crit ? 1.08 : 0.96) : 0.72, trail:strong, pulse:d.crit ? 'crit' : 'hit', tag:strong });
          fxCount++;
        }
      ...
      if(heroCastEl) showSkillImpactFx(heroCastEl, monsterFloatAnchor(mon), sk, { skillKey, actor:'hero', amount:dd, targetMax:mon.hpMax, crit:d.crit || forceCrit, scale:(d.crit||forceCrit)?1.14:1, pulse:(d.crit||forceCrit)?'crit':'hit' });
...
function trackBossCastOutcome(cast,mon,meta){
  ...
  dmgStats.lastBossCastThreat=normalizeTrackedSkillLabel(cast?.threat)||'';
  dmgStats.lastBossCastEmpowered=!!cast?._empowered;
  if(!Array.isArray(dmgStats.bossCastHistory)) dmgStats.bossCastHistory=[];
  dmgStats.bossCastHistory.unshift({at:t,boss:dmgStats.lastBossCastBoss,name:dmgStats.lastBossCastName,target:dmgStats.lastBossCastTarget,damage,kind:dmgStats.lastBossCastKind,empowered:!!cast?._empowered,threat:normalizeTrackedSkillLabel(cast?.threat)||''});
  dmgStats.bossCastHistory.splice(3);
}
...
function combatCueToast(title, detail, kind){
  ...
  const now=Date.now();
  const key=kind||'info';
  const gap=key==='danger'?1100:key==='crit'?1000:1400;
  if((_combatCueCooldown[key]||0)>now)return;
  _combatCueCooldown[key]=now+gap;
  const st=document.getElementById('stage');if(!st)return;
  const el=document.createElement('div');
  el.className='combat-cue-toast '+key;
  const b=document.createElement('b');
  b.textContent=String(title||'战斗提示');
  el.appendChild(b);
  if(detail){
    const span=document.createElement('span');
...
const _combatMomentCooldown = {};
function combatMomentCue(kind, title, detail, opts){
  if(typeof document==='undefined'||document.hidden)return;
  const key=String(kind||'info').replace(/[^a-z0-9_-]/gi,'')||'info';
  const now=Date.now();
  const gap=opts?.gap || (key==='crit'?2200:key==='heal'||key==='shield'?2600:2400);
  if((_combatMomentCooldown[key]||0)>now)return;
  _combatMomentCooldown[key]=now+gap;
  if(key==='crit'||key==='hit')stageFlashFx('crit');
  if(key==='heal'||key==='shield')stageEdgeFx(key,{intensity:.72});
  combatEventBanner(title, detail, key);
}
function combatMomentMaxHp(src){
  if(src==='hero')return Math.max(1,state?.hero?.hpMax||1);
  const st=(typeof computeCompanionStats==='function')?computeCompanionStats():null;
  return Math.max(1,st?.hpMax||state?._compHpMax||state?.hero?.hpMax||1);
}
function maybeCombatDamageMoment(src, amt, isCrit, cleanLabel, prevMax){
  if(!prevMax || amt<=prevMax)return;
  const hitCount=dmgStats[src==='hero'?'heroHits':'compHits']||0;
  if(hitCount<4)return;
  const hpRef=combatMomentMaxHp(src);
  const recordJump=amt>=Math.max(prevMax*1.28,prevMax+Math.max(10,hpRef*.025));
  const heavyHit=amt>=hpRef*.18;
  if(!recordJump&&!heavyHit)return;
  const owner=src==='hero'?'主角':'随从';
  const title=isCrit?`${owner}大暴击`:`${owner}爆发`;
  combatMomentCue(isCrit?'crit':'hit',title,(cleanLabel?cleanLabel+' · ':'')+fmt(amt),{gap:isCrit?2100:2600});
}
function maybeCombatSupportMoment(src, kind, amt, cleanLabel, prevMax){
  if(!prevMax || amt<=prevMax)return;
  const hpRef=combatMomentMaxHp(src);
  const recordJump=amt>=Math.max(prevMax*1.25,prevMax+Math.max(10,hpRef*.025));
  const bigSave=amt>=hpRef*(kind==='heal'?0.14:0.16);
  if(!recordJump&&!bigSave)return;
  const owner=src==='hero'?'主角':'随从';
  const title=kind==='heal'?`${owner}大治疗`:`${owner}厚护盾`;
  combatMomentCue(kind,title,(cleanLabel?cleanLabel+' · ':'')+fmt(amt),{gap:2800});
}
...
function trackHeal(src,amt,skillLabel){
  amt=Math.floor(amt||0);if(amt<=0)return;
  const t=Date.now();if(!dmgStats.start)dmgStats.start=t;dmgStats.last=t;
  const isHero=src==='hero';
  const totalKey=isHero?'heroHeal':'compHeal';
  const maxKey=isHero?'heroHealMax':'compHealMax';
  const skKey=isHero?'heroHealSkills':'compHealSkills';
  const prevMax=dmgStats[maxKey]||0;
  dmgStats[totalKey]=(dmgStats[totalKey]||0)+amt;
  if(amt>(dmgStats[maxKey]||0))dmgStats[maxKey]=amt;
  const cleanLabel=normalizeTrackedSkillLabel(skillLabel);
...
function trackShield(src,amt,skillLabel){
  ...
  const t=Date.now();if(!dmgStats.start)dmgStats.start=t;dmgStats.last=t;
  const isHero=src==='hero';
  const totalKey=isHero?'heroShield':'compShield';
  const maxKey=isHero?'heroShieldMax':'compShieldMax';
  const prevMax=dmgStats[maxKey]||0;
  dmgStats[totalKey]=(dmgStats[totalKey]||0)+amt;
  if(amt>(dmgStats[maxKey]||0))dmgStats[maxKey]=amt;
  const cleanLabel=normalizeTrackedSkillLabel(skillLabel);
